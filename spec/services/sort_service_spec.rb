...
describe SortService do
  ...
  let(:user) { FactoryBot.create(:user, organization: course.organization) }

  it 'should order lessons properly' do
    order_params = { '0' => { id: lesson1.id, position: 2 }, '1' => { id: lesson2.id, position: 1 } }
    SortService.sort(model: Lesson, order_params: order_params, attribute_key: :lesson_order, user: user)
